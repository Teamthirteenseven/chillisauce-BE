...
public class SpaceResponseDto {
    ...
    public SpaceResponseDto(Space space, Long floorId, String floorName, List<Object[]> locationsWithUserLocations) {
        ...
        this.floorId = floorId;
        this.floorName = floorName;
        this.boxList = locationsWithUserLocations.stream().filter(obj -> obj[0] instanceof Box).map(obj -> {Box box = (Box) obj[0];UserLocation userLocation = (UserLocation) obj[1];
                    return new BoxResponseDto(box, userLocation);}).collect(Collectors.toList());
